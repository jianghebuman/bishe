...
SELECT * FROM (
  SELECT 33,'admin:seeker-post','求职信息管理','MENU',0,'/admin/seeker-post',14
  UNION ALL
...
SELECT * FROM (
  SELECT 1,1,1,'计算机本科求Java后端开发实习','Java后端开发','北京,杭州','8K-12K','熟悉Spring Boot、MySQL和Redis，有校园招聘系统和论坛项目经验，希望参与真实业务后端开发。',1,42
...
SELECT * FROM (
  SELECT 9,3,'简历门诊与一对一求职辅导预约开放','针对秋招同学开放简历一对一优化服务。','<p>本周起，校就业指导中心开放简历门诊预约，帮助同学梳理项目亮点与岗位匹配度。</p>','就业指导中心',128,0,1,'2026-03-09 10:00:00'
...
SELECT * FROM (
  SELECT 7,'字节跳动产品与后端宣讲会',1,'字节跳动科技有限公司','2026-04-20 19:00:00','大学生活动中心A厅','围绕产品协作、后端服务和成长路径展开。',58,1
...
SELECT * FROM (
  SELECT 7,'2026春季互联网与人工智能双选会','2026-04-20 09:00:00','学校体育馆主馆','校就业指导中心','聚焦前端、后端、算法、测试等技术岗位。',95,2600,540,1
...
SELECT * FROM (
  SELECT 9,1,'张三','简历完整度提升了，分享下我的修改思路','最近把简历从 68% 补到 92%，主要做了项目量化和成果提炼。','经验分享',74,14,3,1,1
...
SELECT * FROM (
  SELECT 9,2,'李四','量化成果这一步真的很关键，尤其是项目描述。',4,1
...
SELECT * FROM (
  SELECT 4,'STUDENT','赵六','希望增加算法岗专场筛选。','zhaoliu@stu.com',NULL,NULL,0
...
SELECT * FROM (
  SELECT 2,'STUDENT','收藏岗位提醒','您收藏的Java开发工程师岗位仍在招聘中。','SYSTEM',0
...
SELECT * FROM (
  SELECT 1,'ADMIN','admin','OPERATION','岗位审核','审核通过岗位：Java开发工程师','PUT /admin/job/1/audit','status=1','127.0.0.1',1,NULL,48
...
SELECT * FROM (
  SELECT 29,13,28,'校园咖啡店服务员',2,12,'上海',3,5,'不限','不限','在校生','负责点单、出品协助、门店卫生和顾客引导。','沟通自然，能接受排班，每周到岗3天以上。','包吃,节日福利,晋升培训',1,NULL,1,188,12,'2026-03-29 09:00:00'
