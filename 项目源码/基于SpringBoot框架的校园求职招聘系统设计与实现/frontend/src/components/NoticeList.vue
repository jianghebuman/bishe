...
const typeText = (t) => ({ SYSTEM: '系统', APPLY: '投递', INTERVIEW: '面试', OFFER: 'Offer', AUDIT: '审核', ACTIVITY: '活动', CHAT: '沟通', FORUM: '社区' }[t] || '系统')
const tagType = (t) => ({ APPLY: 'success', INTERVIEW: 'warning', OFFER: 'danger', AUDIT: 'primary', ACTIVITY: 'success', CHAT: 'warning', FORUM: 'success' }[t] || 'info')
...
.icon { width:42px;height:42px;border-radius:50%;background:var(--cr-text-muted);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:600;flex-shrink:0;&.apply,&.activity,&.forum{background:var(--cr-success)}&.interview,&.chat{background:var(--cr-warning)}&.offer{background:var(--cr-danger)}&.audit{background:var(--cr-primary)} }
